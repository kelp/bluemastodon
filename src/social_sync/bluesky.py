"""Bluesky API client for social-sync.
...
class BlueskyClient:
    ...
    def get_recent_posts(self, hours_back: int = 24, limit: int = 20) -> List[BlueskyPost]:
    ...
    def _get_user_profile(self):
        ...
        try:
            return self.client.app.bsky.actor.getProfile({"actor": self.config.username})
        except AtProtocolError as e:
    ...
    def _convert_to_bluesky_post(self, feed_view, profile) -> BlueskyPost:
        ...
        return BlueskyPost(
            ...
            author_display_name=profile.displayName if hasattr(profile, 'displayName') else None,
        ...
        if hasattr(post.record.embed, 'external'):
            ...
            links.append(
                Link(
                    ...
                    image_url=self._get_blob_url(post, ext.thumb.ref.link) if hasattr(ext, 'thumb') else None,
                )
            )
    ...
    def _get_blob_url(self, post: Any, ref: str) -> str:
        ...
        return f"https://bsky.social/xrpc/com.atproto.sync.getBlob?did={post.author.did}&cid={ref}"